@if($car->status === 'prepared')
<div x-data="{ open: false }">
    <button @click="open = true" class="bg-blue-500 text-white px-3 py-1 rounded">Хятад хилд шилжүүлэх</button>
    ...
    <div x-show="open" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md" @click.away="open = false">
            <h2 class="text-lg font-bold mb-4">Хятад бичиг зардал оруулах</h2>
            <form method="POST" action="{{ route('cars.status', $car) }}">
                @csrf
                <div class="mb-4">
                    <label class="block mb-1 font-medium">Зөвшөөрлийн зардал (¥)</label>
                    <input type="number" name="permit_cost_yuan" step="0.01" class="w-full border rounded p-2" required>
                </div>
